...
cu=3.2; %copper volume in cc
nc=cu/7.11; %moles
cn0=3.2e-6*nc; % heat cap = cn0 B^2/T^2

a=64;  %sinter area in m^2

he=.1;  %helium volume in cc
nh=he/36.8; %moles at 0 bar
ch0=nh*24;   %heat cap = ch0 T

%demag  profile
b0=6.4; %initial field
b1=1; %mid f1eld
bf=0.014;% final field

tt=24; % total time in hours
td=3; %demag time, to mid field in hours
tdd= 2; % demag time, mid to final field
sd=td*3600;
sdd=tdd*3600;
ttt=tt*3600; %secs
step=.5;
...
for i=2:length(t)
   ...
   kap(i)=a*(0.15*te(i-1)+4940*te(i-1)^3)/741; %kap link
