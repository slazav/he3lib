...
%  chi = (2 + y0) / (3 + f0a*(2 + y0));
  ...
  for f=freqs;
    ...
    Dxx0= DX .* (kk0xx*t0);
    Dxx2= DX .* (kk0xx*t0 + kk2xx*t2);
    Dxx4= DX .* (kk0xx*t0 + kk2xx*t2 + kk4xx*t4);

    Dzz0= DX .* (kk0zz*t0);
    Dzz2= DX .* (kk0zz*t0 + kk2zz*t2);
    Dzz4= DX .* (kk0zz*t0 + kk2zz*t2 + kk4zz*t4);

%    DX = vf^2./(2*pi*f*chi);

%    Dxx0= DX .* (k0xx*y0);
%    Dxx2= DX .* (k0xx*y0 + k2xx*y2);
%    Dxx4= DX .* (k0xx*y0 + k2xx*y2 + k4xx*y4);

%    Dzz0= DX .* (k0zz*y0);
%    Dzz2= DX .* (k0zz*y0 + k2zz*y2);
%    Dzz4= DX .* (k0zz*y0 + k2zz*y2 + k4zz*y4);

%    plot(ttcr, he3_diff_perp_zz(ttc, p, f)./Dzz, 'r-');
%    plot(ttcr, he3_diff_perp_xx(ttc, p, f)./Dxx, 'b-');

    semilogy(ttcr, he3_diff_perp_xx(ttc, p, f), 'b-', 'linewidth', 2);
    semilogy(ttcr, he3_diff_perp_zz(ttc, p, f), 'r-', 'linewidth', 2);

    semilogy(ttcr, Dxx0, 'c-');
    semilogy(ttcr, Dxx2, 'c-');
    semilogy(ttcr, Dxx4, 'k-');
    semilogy(ttcr, Dzz0, 'm-');
    semilogy(ttcr, Dzz2, 'm-');
