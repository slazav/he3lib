...
%  addpath ../matlab
  ...
  ct=0:0.001:1;
  [xx,yy]=meshgrid(x,ct);
  int = integrand(xx,yy, gap,gap*1.01,gap*0.99,ttc, 0.0);
...
function int = integrand(x,ct,gap0,gap1,gap2,TTc,we)
  xi = 1./x-1;
...
%  xi = x;
  ...
  fEp = @(s) sqrt(xi.^2 + gaps + we^2/4 - s*we*Ez);
  EE  = @(s) TTc*log(1-1./(1+exp(fEp(s)/TTc))) - 0.5*(fEp(s)-xi);
  EE0 = TTc*log(1-1./(1+exp(E0/TTc))) - 0.5*(E0-xi);
...
%% gap in ttc units,  w0,we in gap units!
function v = zerofunc(ttc, gap, f0a, w0, we, A,B)
  we=we*gap; w0=w0*gap;

  f1 = @(x,y) integrand(x,y, gap,gap*A,gap*B,ttc,we);
  v = f1 - (we - w0)^2/(8*f0a);
end
